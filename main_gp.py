...
from utils.dataload_utils import load_gp_data, load_m3_data
...
from utils.analyzer_utils import analyze_then_generate_kernels_first, analyze_then_generate_kernels, no_analyze_then_generate_kernels_first, no_analyze_then_generate_kernels
...
if __name__ == '__main__':    
    ...
    parser.add_argument('-s', '--set_se_const', type=bool, default=False, help='restarts')
    parser.add_argument('-a', '--alpha', type=float, default=50, help='alpha')
    ...
    directory_path = f'./output/test/fig-R{R}-restarts{restarts}'
    ...
    directory_path += f'-{vlm_model_name}-test'
    
    # current_dir = os.path.dirname(os.path.realpath(__file__))
    
    for data_idx in datasets_to_test:
        data_name = datasets[data_idx-1]
        ...
        X, Y, valX, valY, testX, testY, data, range_, origX, origY = load_gp_data(data_name, return_origXY=True, noise=noise)
        ...
        else: #### input vision, no analyze
            k_exemplars, params, logs = no_analyze_then_generate_kernels_first(X, Y, data_name, round=0, model=vlm_model_name, logger=logger) # TODO: 프롬프트 수정
            start_kexs = [parse_kernel_name(each) for each in k_exemplars] #### 랜덤 initialize용 커널 조합
            # start_kex_mods = [parse_kernel_model(each) for each in k_exemplars]
            start_kex_params = [parse_kernel_params(each) for each in k_exemplars]
        ...
        k_exemplars = fit_mods_parallel_processes(X, Y, start_kexs, restarts, optimiser, max_retries)
            ...
            for ii, name in enumerate(fgk_dict.keys()):
                ...
                if not name in genetic_pool_dict:
                    genetic_pool_dict[name] = {
                        ...
                        'score': int(np.mean(score_for_criteria.get('score for match', 0)) + np.mean(score_for_criteria.get('score for confidence', 0)) + np.mean(score_for_criteria.get('score for consistency', 0))),
                        'model': model,
        ...
        for i in range(1, R):
            ...
            for each_exemplars, each_exemplars_string in zip(k_exemplars, k_exemplars_string):
                ...
                if USE_ANALYZER:
                    ...
                    tmp_response, _, model_logs = no_analyze_then_generate_kernels([each_exemplars], [each_exemplars_string], X=X, Y=Y, data_name=data_name, round=i, use_changepoint=False, model=vlm_model_name, logger=logger)
                    tot_tmp_response.extend(tmp_response)
                    # tmp_response_cp, model_logs_cp = analyze_then_generate_kernels([each_exemplars], [each_exemplars_string], X=X, Y=Y, data_name=data_name, round=i, use_changepoint=True, model=vlm_model_name, logger=logger)
                    # tot_tmp_response_cp.extend(tmp_response_cp)
                    tmp_response_cp, tot_tmp_response_cp = [], []
            ...
            ##### PLEASE HIERARCHICALLY MANAGE THE ISLANDS
                ...
                print('tuning done and start again2')
                # else: 
                ...
                for ii, name in enumerate(fgk_dict.keys()):
                    ...
                    if not name in genetic_pool_dict:
                        genetic_pool_dict[name] = {
                            ...
                            'score for confidence': score_for_criteria.get('score for confidence', 0),
                            'score for consistency': score_for_criteria.get('score for consistency', 0),
                            'score': np.mean(score_for_criteria.get('score for match', 0)) + np.mean(score_for_criteria.get('score for confidence', 0)) + np.mean(score_for_criteria.get('score for consistency', 0)),
                            'model': model,
                            'likelihood': likelihood,
                            'BIC': bic,
                            'val mse': val_mse,
